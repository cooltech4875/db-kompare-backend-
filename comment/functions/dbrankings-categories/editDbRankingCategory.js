// Edit/Update DB Ranking Category Handler
// This endpoint updates a DB Ranking Category by ID
...
export const handler = async (event) => {
  try {
    ...
    const updated = await updateItemInDynamoDB({
      table: TABLE_NAME.DB_RANKING_CATEGORIES,
      Key: { id },
    ...
    return sendResponse(
      200,
      "DB Ranking Category updated successfully",
      updated.Attributes
    );
  } catch (error) {
    console.error("Error updating DB Ranking Category:", error);
