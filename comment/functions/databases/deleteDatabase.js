// Delete Database Handler
// This endpoint deletes a Database by ID

import DynamoDB from "aws-sdk/clients/dynamodb.js";
const DynamoDBClient = new DynamoDB.DocumentClient();

import { sendResponse, getItem } from "../../helpers/helpers.js";
import { TABLE_NAME } from "../../helpers/constants.js";

export const handler = async (event, context, callback) => {
  try {
    // Extract database ID from request body
    const { id } = JSON.parse(event.body || "{}");

    if (!id) {
      return sendResponse(400, "Missing database ID", null);
    }

    // Verify database exists
    const existing = await getItem(TABLE_NAME.DATABASES, { id });
    if (!existing || !existing.Item) {
      return sendResponse(404, "Database not found", null);
    }

    // Delete the database from DynamoDB
    const deleteParams = {
      TableName: TABLE_NAME.DATABASES,
      Key: {
        id: id,
      },
      ReturnValues: "ALL_OLD",
    };

    const deleted = await DynamoDBClient.delete(deleteParams).promise();

    if (!deleted.Attributes) {
      return sendResponse(404, "Database not found or already deleted", null);
    }

    return sendResponse(200, "Database deleted successfully", deleted.Attributes);
  } catch (error) {
    console.error("Error deleting Database:", error);
    return sendResponse(500, "Internal Server Error", error.message);
  }
};

